// import the data from data.js
	...
	function updateFilters() {
	  ...
	  let filtername = ["#datetime", "#city", "#state", "#country", "#shape"];
	  let filterval;
	  filters = {};
	

	  for (filterval of filtername)
	  {
	    if(d3.select(filterval).property("value"))
	    {
	      // If a filter value was entered then add that filterId and value
	      // to the filters list. Otherwise, clear that filter from the filters object
	      filters[filterval] = d3.select(filterval).property("value");
	    }    
	  }
